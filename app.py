...
# metadata.json generation or loading
...
if os.path.exists(metadata_file):
    ...
    metadata_map = {}
    # list PDF files in PAPERS_PATH
    for fname in os.listdir(PAPERS_PATH):
        if not fname.lower().endswith('.pdf'):
            continue
        pid = fname
        # minimal metadata: use filename as title placeholder
        metadata_map[pid] = {"title": pid}
    # write out for future runs
    with open(metadata_file, "w") as f:
        json.dump(metadata_map, f, indent=2)
st.session_state.metadata_map = metadata_map

# ——————— Rate-limited, Retrying CLI-based Ask Function ———————, Retrying Ask Function ———————
